...
export default function Sidebar() {
    return (
        <aside className="flex flex-col p-4 h-full">
            <figure className="w-14 h-14 bg-gray-800 rounded-xl animate-pulse mx-auto text-white font-bold flex justify-center items-center -rotate-90">{"</>"}</figure>
            ...
            <ul className="flex justify-between text-2xl mt-6 text-gray-400">
                {socialsItems.map((social, index) => (
                    <li key={index}>
                        <a href={social.url} target="_blank" className="hover:text-blue-400 duration-200">
                            {social.icon}
                        </a>
                    </li>
                ))}
            ...
            {/* <a href="https://satyamahendra.gumroad.com/l/deadsimpleportfolio" target="_blank" className="text-xs hover:underline text-gray-400 text-center mt-2">
                like the template? get it here!
            </a> */}
        </aside>
    )
}
